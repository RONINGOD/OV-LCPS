...
class Asymm_3d_spconv(nn.Module):
    ...
    def forward(self, voxel_features, coors, batch_size, fusion_dict):
        coors = coors.int() 
        ret = spconv.SparseConvTensor(voxel_features, coors, self.sparse_shape,
                                      batch_size) # [13750, 16]
        ret = self.downCntx(ret) # [13750, 32]
        ...
        logits = logits.dense() # [1, 17, 480, 360, 32]

        up1e_ins_heatmap = self.upBlock3_ins_heatmap(up2e, down1b)
        ...
        up1e_ins_offset = self.upBlock3_ins_offset(up2e, down1b)
        ...
        up1e_ins_instmap = self.upBlock3_ins_instmap(up2e, down1b)
        ...
        instmap = up0e_ins_instmap.dense().squeeze(-1) # [1, 32, 480, 360]
        instmap = F.pad(instmap, (1, 1, 0, 0), mode='circular') # [1, 32, 480, 362]
        instmap = self.logits_instmap(instmap) # [1, 2, 480, 360]

        return logits, heatmap, offset, instmap
