...
from network.OVPFC import OVPFC
...
def main():
    ...
    parser.add_argument('-w', '--work_dir', default='work_dir/nusc_ovpfc_launch')
    ...
    while epoch < cfg['model']['max_epoch']:
        if start_train:
            ...
            for i_iter, data in enumerate(train_dataset_loader):
                ...
                global_iter += 1
                if i_iter % print_freq == 0 and args.local_rank < 1:
...
# python -m torch.distributed.launch --nproc_per_node=2 --use_env train_ovpfc_launch.py -c configs/open_pa_po_nuscenes_pfc.py -w work_dir/nusc_ovpfc_launch